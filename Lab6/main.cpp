...
int main() 
{
    ...
    std::cout << "Select an action: load, save, add, show, fight. " << std::endl;
    ...
    do 
    {
        ...
        else if (query == "add") 
        {
            ...
            if (new_npc) persons.push_back(new_npc);
        }
        else if (query == "show") 
        {
    ...
    {
        for (auto& attacker : persons) defender->accept(attacker.get(), distance);
    }
}
